...
class Fold:
    ...
    def __enter__(self):
        print(
            'travis_fold:start:%s\033[33;1m%s\033[0m' % (self.fold_name,
                                                         self.description),
            flush=True)
...
def upload_to_gcs(patterns, dest):
    ...
    for pattern in patterns:
        ...
        for filepath in glob.glob(pattern):
            ...
            blob.make_public()
            print('uploaded ', blob.public_url.replace(
                '%2F',
...
def test():
    ...
    args = []
    if os.environ['TRAVIS_BRANCH'] != 'master' or os.environ[
...
def main():
    ...
    with Fold('script.build', 'Building'):
        if os_name == 'osx':
            cmake_options = [
                '-DCMAKE_PREFIX_PATH=/usr/local/opt/qt', '-DBUILD_UI=ON'
            ]
        elif os_name == 'linux':
            cmake_options = [
                '-DBUILD_MANYLINUX=ON',
                '-DPYTHON_INCLUDE_DIR=%s/include/python%sm' %
                (sys.base_prefix, os.environ['PYVER'])
            ]
    ...
    if cross not in ('win64', 'win32'):
        with Fold('script.test', 'Running tests'):
            call(['ctest', '--verbose', '-E',
                  '\.test'])  # Exclude libzip tests
